...
def tokenize(batch, tokenizer, device, context_templates=None, hparams=None):
    assert context_templates is not None and len(context_templates) > 0
    len_temp = len(context_templates)

    # === 用你数据集的字段 ===
    prompts = [item['prompt'] for item in batch] 
    labels = [item['target_new'] for item in batch] 
    loc_prompts = [item['subject'] for item in batch]
    ...
    if getattr(hparams, 'use_chat_template', False):
        ...
        prompt_ids = tokenizer([
            ...
            for templ in context_templates for p in prompts
    ...
    if getattr(hparams, 'objective_optimization', None) == 'only_label':
    ...
    for i, loc_prompt in enumerate(loc_prompts):
        ...
        if loc_prompt and (loc_prompt in prompts[i]):
            rows = tokens['input_ids'][int(i*len_temp):int((i+1)*len_temp)]
            act_mask = torch.zeros_like(rows)
            deact_mask = torch.zeros_like(rows)
            ...
            subj_token_variants = []
            for t in subj_text_variants:
                ids = tokenizer.encode(t, add_special_tokens=False)
                if ids:
                    subj_token_variants.append(ids)

            if len(subj_token_variants) == 0:
                sample_act, sample_deact = None, None
            else:
                found_any = False
                for j, token_row in enumerate(rows):
                    row_list = token_row.detach().cpu().tolist()

                    start_idx = None
                    matched_len = None
                    for cand in subj_token_variants:
                        start_idx = find_sublist_start_index(row_list, cand)
                    ...
                    if start_idx is not None:
                        found_any = True
                        act_mask[j][start_idx: start_idx + matched_len] = 1
                        if start_idx > 0:
                            deact_mask[j][:start_idx] = 1
                        deact_mask[j][start_idx + matched_len:] = 1
                    else:
                        pass

                if found_any:
                    sample_act, sample_deact = act_mask, deact_mask
                else:
                    sample_act, sample_deact = None, None
        else:
            sample_act, sample_deact = None, None

        act_masks.append(sample_act)
        deact_masks.append(sample_deact)

    act_masks  = [m.to(device) if m is not None else None for m in act_masks]
    deact_masks = [m.to(device) if m is not None else None for m in deact_masks]
