...
def brackets_to_periods(name: str) -> str:
    """
    将 'layers[27].mlp.down_proj' 转为 'layers.27.mlp.down_proj'
    仅替换数字下标形式的方括号。
    """
...
class WISE(torch.nn.Module):
    ...
    def reset_layer(self):
        """
        返回 {param_full_name: tensor}，用于把 WISEAdapter 的 new_weight
        回滚为原始层的权重。
        """
        adapter = self.get_adapter_layer()
        weights_copy = {}
    
        # 关键：把 `layers[27]` 这类转成 `layers.27`
        name_base = brackets_to_periods(self.layer)  # 如 "model.layers.27.mlp.down_proj"
        key_weight = f"{name_base}.new_weight"
    
        # 可选：防御性检查，便于调试路径是否存在
        # 如果找不到这个名字，再尝试去掉可能多余的前缀 "model."
        try_names = [key_weight]
        if key_weight.startswith("model."):
            try_names.append(key_weight[len("model."):])
        else:
            try_names.append("model." + key_weight)
    
        # 选择一个真正存在于 named_parameters() 的名字
        ...
        for cand in try_names:
            if cand in params:
                ...
                break
        if hit_name is None:
            # 退一步：找一个以 ".new_weight" 结尾并且包含层名片段的参数（容错）
            for n in params.keys():
                if n.endswith(".new_weight") and all(seg in n for seg in name_base.split(".")):
                    hit_name = n
                    break
    
        if hit_name is None:
            # 仍然找不到就抛错（会提示现有候选可用名字）
            available = [n for n in params.keys() if n.endswith(".new_weight")]
            raise LookupError(
                f"Cannot locate new_weight param for layer '{self.layer}'. "
                f"Tried: {try_names}. "
                f"Available *.new_weight: {available[:8]}{' ...' if len(available)>8 else ''}"
            )
    
        # 用原始层的 weight 作为回滚目标
        weights_copy[hit_name] = adapter.original_layer.weight.detach().clone().cpu()
        return weights_copy
