...
class ULTRAEDIT(EditableModel):
    ...
    def cache(self, batch):
        if not isinstance(batch, (list, tuple)) or len(batch) == 0:
            raise ValueError("ULTRAEDIT.cache(): received empty batch; module_kv_map would be empty.")
        ...
        for idx, t in enumerate(batch):
            with TracerDict(self.model, self.config, t) as tr:
                ...
                if not torch.isfinite(loss):
                    LOG.warning(f"Non-finite loss at sample {idx}: {loss}")
                loss.backward()
    
            missing = [m for m in self.config.inner_params if m not in tr]
            if missing:
                LOG.warning(f"TracerDict missing modules for sample {idx}: {missing}")
    
            for module_idx, module_name in enumerate(self.config.inner_params):
                if module_name not in tr:
                    continue  # 保守处理：该样本缺这个模块就跳过
        ...
        if len(module_kv_map) == 0:
            raise RuntimeError("ULTRAEDIT.cache(): module_kv_map is empty; check batch, labels, and TracerDict hooks.")
